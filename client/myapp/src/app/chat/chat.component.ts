...
export class ChatComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit() {
    ...
    this.socket.on('new-message', function (data) {
      let roomuser=JSON.parse(localStorage.getItem("user"));
     console.log('hello'+roomuser)
      if(data.message.room === roomuser.room) {
        this.chats.push(data.message);
        this.msgData = { room: roomuser.room, nickname: user.nickname, message: '' }
        this.scrollToBottom();
      }
    }.bind(this));
  }
